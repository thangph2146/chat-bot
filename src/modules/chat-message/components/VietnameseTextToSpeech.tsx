...
interface VietnameseTextToSpeechProps {
  text: string;
  autoPlay?: boolean;
  className?: string;
...
export const VietnameseTextToSpeech: React.FC<VietnameseTextToSpeechProps> = ({ 
  text, 
  autoPlay = false,
  className = "",
  ...
  const [rate, setRate] = useState(0.9); // Chậm hơn cho tiếng Việt
  ...
  useEffect(() => {
    ...
    const loadVoices = () => {
      ...
      // Tự động chọn voice tiếng Việt mặc định
  ...
  const handleLanguageChange = (lang: 'vi' | 'en') => {
    ...
    if (lang === 'vi') {
      ...
      if (vietnameseVoices.length > 0) {
        ...
        setRate(0.9); // Chậm hơn cho tiếng Việt
      }
    } else {
      const englishVoices = getEnglishVoices();
      if (englishVoices.length > 0) {
        setVoice(englishVoices[0]);
        setRate(1); // Tốc độ bình thường cho tiếng Anh
      }
    }
  };
