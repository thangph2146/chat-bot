...
2.  **Cấu hình API Endpoints:**
    *   Mở file `js/chat/config.js`.
    *   Tìm và thay đổi các giá trị của các hằng số sau để trỏ đến API backend của bạn:
        *   `AUTH_LOGIN_ENDPOINT`: URL cho API đăng nhập.
        *   `CHAT_SEND_ENDPOINT`: URL cho API gửi tin nhắn chat.
        *   `CHAT_HISTORY_ENDPOINT`: URL cho API lấy lịch sử chat (bao gồm placeholder cho `conversationId`).
        *   `CHAT_CONVERSATIONS_ENDPOINT`: URL cho API lấy danh sách hội thoại.
        *   *(Tùy chọn)* `API_VALIDATE_TOKEN_ENDPOINT`: URL cho API kiểm tra token (nếu có).
    *   Ví dụ:
        ```javascript
        // js/chat/config.js
        export const AUTH_LOGIN_ENDPOINT = 'http://localhost:3000/api/v1/auth/login';
        export const CHAT_SEND_ENDPOINT = 'http://localhost:3000/api/v1/chat/send';
        // Lưu ý: Sử dụng template literal nếu endpoint chứa biến
        export const CHAT_HISTORY_ENDPOINT = (conversationId) => `http://localhost:3000/api/v1/chat/history/${conversationId}`;
        export const CHAT_CONVERSATIONS_ENDPOINT = 'http://localhost:3000/api/v1/chat/conversations';
        ```

3.  **Chạy Backend:** Đảm bảo rằng API backend của bạn đang chạy và có thể truy cập được từ các URL đã cấu hình ở trên. Backend cần xử lý các yêu cầu API được mô tả trong **Mục 5**.

4.  **Mở Frontend:**
    *   Bạn có thể mở file `login.html` trực tiếp trong trình duyệt.
    *   Hoặc, để có trải nghiệm tốt hơn (ví dụ: tránh một số vấn đề CORS tiềm ẩn khi phát triển cục bộ), bạn có thể sử dụng một máy chủ web đơn giản. Nếu bạn có Node.js và npm/npx, bạn có thể chạy:
        ```bash
        npx live-server
        ```
        Lệnh này sẽ khởi động một máy chủ cục bộ và tự động mở dự án trong trình duyệt của bạn. Nó cũng sẽ tự động tải lại trang khi bạn thay đổi file.

5.  **Sử dụng Ứng dụng:**
    *   Truy cập trang đăng nhập (thường là `http://127.0.0.1:8080/login.html` nếu dùng `live-server`).
    *   Đăng nhập bằng tài khoản hợp lệ đã có trên hệ thống backend.
    *   Sau khi đăng nhập thành công, bạn sẽ được chuyển hướng đến trang chat (`index.html`).
...
Ứng dụng cần duy trì trạng thái đăng nhập của người dùng giữa các lần tải trang và đảm bảo chỉ người dùng đã xác thực mới có thể truy cập các trang được bảo vệ (như trang chat). Cơ chế này hoạt động như sau:

1.  **Lưu trữ Dữ liệu Phiên:**
    *   Khi người dùng đăng nhập thành công (API `/login` trả về thành công), thông tin xác thực quan trọng (ít nhất là **token** truy cập, thường là JWT, và có thể cả **userId** hoặc thông tin người dùng cơ bản khác) được lưu vào `localStorage` của trình duyệt.
    *   Dữ liệu này thường được lưu dưới dạng một chuỗi JSON, sử dụng một khóa (key) cố định được định nghĩa trong `js/chat/auth.js` (ví dụ: `const USER_DATA_KEY = 'hubAiUserData';`).
    *   Ví dụ về dữ liệu lưu trong `localStorage`:
        ```json
        // Key: hubAiUserData
        // Value: {"token":"eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...", "userId":"user123", "email":"user@example.com"}
        ```

2.  **Kiểm tra Xác thực khi Tải Trang:**
    *   Đối với các trang yêu cầu xác thực (ví dụ: `index.html`), script chính (như `main.js`) sẽ thực thi **ngay khi bắt đầu**.
    *   Bước đầu tiên trong `main.js` là gọi hàm `checkAuthentication()` từ `js/chat/auth.js`.
    *   Hàm `checkAuthentication()` thực hiện các việc sau:
        *   Đọc dữ liệu từ `localStorage` bằng `USER_DATA_KEY`.
        *   Kiểm tra xem dữ liệu có tồn tại và có hợp lệ không (ví dụ: parse JSON thành công, có chứa trường `token`).
        *   **Nếu dữ liệu không hợp lệ hoặc không tồn tại:** Người dùng chưa đăng nhập hoặc phiên đã hết hạn/bị xóa. Hàm sẽ thực hiện chuyển hướng ngay lập tức đến trang đăng nhập (`login.html`) bằng cách gán `window.location.href = 'login.html';`. Quá trình khởi tạo trang hiện tại sẽ bị dừng lại.
        *   **Nếu dữ liệu hợp lệ:** Hàm trả về đối tượng chứa thông tin người dùng (ví dụ: `{ token, userId, email }`). Script `main.js` nhận được dữ liệu này và tiếp tục quá trình khởi tạo giao diện chat.

3.  **Sử dụng Token cho Yêu cầu API:**
    *   Mọi yêu cầu API đến các endpoint được bảo vệ (ví dụ: gửi tin nhắn, lấy lịch sử) đều phải đính kèm token xác thực.
    *   Hàm tiện ích `fetchWithAuth` trong `js/chat/api.js` được thiết kế để tự động hóa việc này:
        *   Trước khi thực hiện `fetch`, nó gọi hàm `getUserData()` từ `js/chat/auth.js` để lấy đối tượng dữ liệu người dùng từ `localStorage`.
        *   Nếu lấy được token, nó sẽ thêm header `Authorization: Bearer <token>` vào yêu cầu `fetch`.
        *   Nếu không tìm thấy token (người dùng chưa đăng nhập), `fetchWithAuth` *nên* xử lý tình huống này, có thể bằng cách ném lỗi hoặc trả về một giá trị đặc biệt để ngăn yêu cầu được gửi đi.

4.  **Xử lý Token Hết hạn / Không hợp lệ từ API:**
    *   Backend sẽ xác thực token đi kèm trong mỗi yêu cầu API. Nếu token hết hạn, không hợp lệ hoặc bị thu hồi, backend sẽ trả về lỗi `401 Unauthorized`.
    *   Hàm `fetchWithAuth` (hoặc logic xử lý response trong `chat.js` / `chat-page.js`) cần bắt lỗi này.
    *   Khi nhận được lỗi 401:
        *   Hàm `logout()` trong `js/chat/auth.js` được gọi.
        *   `logout()` sẽ xóa dữ liệu người dùng khỏi `localStorage` (`localStorage.removeItem(USER_DATA_KEY);`).
        *   Sau đó, `logout()` chuyển hướng người dùng về trang đăng nhập (`window.location.href = 'login.html';`). Điều này đảm bảo người dùng không bị kẹt ở trang yêu cầu xác thực với token không hợp lệ.

5.  **Đăng xuất:**
    *   Ứng dụng nên cung cấp một nút hoặc liên kết "Đăng xuất".
    *   Khi người dùng nhấn vào đó, trình xử lý sự kiện sẽ gọi trực tiếp hàm `logout()` từ `js/chat/auth.js` để xóa dữ liệu `localStorage` và chuyển hướng về trang đăng nhập.
