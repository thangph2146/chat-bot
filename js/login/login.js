import { AUTH_LOGIN_ENDPOINT, AUTH_GOOGLE_LOGIN_ENDPOINT } from '../chat/config.js';
 ...
 * @param {string} googleIdToken ID Token nhận được từ Google.
 * @returns {Promise<{success: boolean, message?: string}>}
 */
export async function handleGoogleLogin(googleIdToken) {
    console.log('[login.js] Attempting Google login...');
    const apiUrl = AUTH_GOOGLE_LOGIN_ENDPOINT;
    ...
    try {
        ...
        console.log('[login.js] API Google Login Success Data Received:', responseData);

        // Validation cấu trúc response
        if (!responseData || !responseData.data || !responseData.data.token || typeof responseData.data.userId === 'undefined') {
            console.error('[login.js] Google API response missing expected data structure (data.token, data.userId).', responseData);
            return { success: false, message: 'Dữ liệu đăng nhập Google trả về không hợp lệ.' };
        }

        try {
            console.log(`[login.js] Storing Google user data to localStorage with key: ${USER_DATA_KEY}`);
            localStorage.setItem(USER_DATA_KEY, JSON.stringify(responseData));
            console.log('[login.js] Google user data stored successfully.');
            return { success: true };
        } catch (storageError) {
            console.error('[login.js] Error storing Google login info to localStorage:', storageError);
            return { success: true, message: 'Đăng nhập Google thành công nhưng lỗi lưu phiên.' };
        }
    } catch (error) {
        console.error('[login.js] Google Login failed:', error);
        localStorage.removeItem(USER_DATA_KEY);
        return { success: false, message: error.message || 'Lỗi kết nối hoặc xử lý phía máy chủ Google.' };
    }
}
